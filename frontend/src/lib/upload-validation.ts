 ...
 * directly into a `toast.error(uiText('Upload failed'), { description })`.
 *
...
export const validateUploadBatch = (files: readonly File[], limits: UploadValidationLimits): null | string => {
    if (files.length > limits.maxFiles) {
        return uiText('Too many files: max {max} per upload', { max: limits.maxFiles });
    }
    ...
    for (const file of files) {
        if (file.size > maxBytesPerFile) {
            return uiText('File "{name}" is larger than {max} MB', { max: limits.maxFileSizeMb, name: file.name });
        }
    ...
    if (totalBytes > maxTotalBytes) {
        return uiText('Total upload size exceeds the {max} MB limit', { max: limits.maxTotalSizeMb });
    }
