...
type GraphitiSearchAction struct {
	...
	Query          string   `json:"query" jsonschema:"required" jsonschema_description:"Technical-channel payload — natural language query against the team's temporal knowledge graph. REQUIRED for EVERY search_type, including entity_relationships/entity_by_label: it re-ranks/filters the graph traversal, it is never optional just because center_node_uuid or node_labels is set. ALWAYS written in the engagement language declared by your system prompt — the graph stores its episodes in that language, so a query written in another language fails to retrieve stored episodic memory."`
...
type SearchGuideAction struct {
	Questions Strings `json:"questions" jsonschema:"required,type=array,minItems=1,maxItems=5" jsonschema_description:"Technical-channel payload — 1 to 5 detailed, context-rich semantic queries for the team's guide vector store. Must be a real JSON array of strings, e.g. [\"query 1\",\"query 2\"] - NOT a JSON-encoded string containing an array. Write the queries in the engagement language declared by your system prompt — the store keeps its guides in that language, so a query written in another language retrieves nothing. Each query should include scenario context, objectives, and specific intent. Note: The 'Type' field acts as a strict filter."`
...
type StoreGuideAction struct {
	Guide    string `json:"guide" jsonschema:"required" jsonschema_description:"Technical-channel payload — ready guide in markdown format that will be stored in the team's vector store for future retrieval. Write it in the engagement language declared by your system prompt — the same language the queries searching this store are written in; content stored in another language becomes unreachable to future searches. Anonymize all sensitive data (IPs, domains, credentials, paths) using descriptive placeholders."`
	Question string `json:"question" jsonschema:"required" jsonschema_description:"Technical-channel payload — question that was used to prepare this guide; co-indexed with the guide. Written in the engagement language declared by your system prompt."`
...
type SearchAnswerAction struct {
	Questions Strings `json:"questions" jsonschema:"required,type=array,minItems=1,maxItems=5" jsonschema_description:"Technical-channel payload — 1 to 5 detailed, context-rich semantic queries for the team's answer vector store. Must be a real JSON array of strings, e.g. [\"query 1\",\"query 2\"] - NOT a JSON-encoded string containing an array. Write the queries in the engagement language declared by your system prompt — the store keeps its answers in that language, so a query written in another language retrieves nothing. Each query should include the context, what you want to find, what you intend to do with the information, and why you need it. Note: The 'Type' field acts as a strict filter."`
...
type StoreAnswerAction struct {
	Answer   string `json:"answer" jsonschema:"required" jsonschema_description:"Technical-channel payload — ready answer in markdown format that will be stored in the team's vector store for future retrieval. Write it in the engagement language declared by your system prompt — the same language the queries searching this store are written in; content stored in another language becomes unreachable to future searches. Anonymize all sensitive data (IPs, domains, credentials) using descriptive placeholders."`
	Question string `json:"question" jsonschema:"required" jsonschema_description:"Technical-channel payload — question that was used to prepare this answer; co-indexed with the answer. Written in the engagement language declared by your system prompt."`
...
type SearchCodeAction struct {
	Questions Strings `json:"questions" jsonschema:"required,type=array,minItems=1,maxItems=5" jsonschema_description:"Technical-channel payload — 1 to 5 detailed, context-rich semantic queries for the team's code vector store. Must be a real JSON array of strings, e.g. [\"query 1\",\"query 2\"] - NOT a JSON-encoded string containing an array. Write the queries in the engagement language declared by your system prompt — the store keeps its code explanations and questions in that language, so a query written in another language retrieves nothing. Each query should include the context, what you intend to achieve with the code, and the functionality or content that should be included."`
...
type StoreCodeAction struct {
	...
	Question    string `json:"question" jsonschema:"required" jsonschema_description:"Technical-channel payload — question that was used to prepare or to write this code; co-indexed with the code in the team's vector store. Written in the engagement language declared by your system prompt."`
	...
	Explanation string `json:"explanation" jsonschema:"required" jsonschema_description:"Technical-channel payload — fully detailed explanation of the code sample (what it does, how it works, why it is useful, libraries/tools used). Write it in the engagement language declared by your system prompt: the explanation is co-indexed with the code in the team's vector store and searched in that language, so an explanation in another language becomes unreachable to future searches. The code sample itself stays in its original language."`
	Description string `json:"description" jsonschema:"required" jsonschema_description:"Technical-channel payload — short description of the code sample as a summary of the explanation; co-indexed in the team's vector store. Written in the engagement language declared by your system prompt."`
