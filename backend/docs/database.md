...
The knowledge query API excludes `doc_type = 'memory'` from user-managed knowledge listings/searches. Deleting a flow issues an explicit best-effort deletion of **every** row carrying that `flow_id` (`DeleteFlowDocuments`) — long-term memory plus the `answer`/`guide`/`code` knowledge entries the flow's agents stored; documents created by hand in the knowledge base carry no `flow_id` and are left alone. Purging a flow from the recycle bin repeats the same cleanup. This is application behavior, not a database trigger or foreign-key cascade. The knowledge migration intentionally does not drop the LangChain tables on downgrade because they may contain production data managed by the vector store.
...
| `knowledge.sql` | LangChain pgvector documents | Admin/user get/list/update/delete, cosine search, insert, `DeleteFlowDocuments` |
