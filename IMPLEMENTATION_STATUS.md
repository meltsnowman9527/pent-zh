...
- P0A 验收项：① 页面走查**已完成**（见「P0/P0A 验收补齐」：27 条路由 + 390/768/1280 三断点，0 截断 / 0 溢出 / 0 控制台错误，修掉 4 处 axe critical/serious 并加了回归测试）；② 报告 PDF 已确认为完整文档（100 页 / 7294 处文字绘制 / 6 个字体子集，中文由 `NotoSansSC` 承载），但**表头逐项视觉核对仍未做**；③ 无障碍目前是 axe（wcag2a/2aa/21a/21aa/22aa）自动扫描 + 未登录上下文复扫，**没有用真实读屏工具（NVDA/VoiceOver）实测**。计划里的“错误码映射为中文说明、原始诊断放可展开详情”已实现（见 `lib/errors.ts` 与 `ErrorState`），但后端新增错误码时需要同步登记，否则会退回展示英文原文。
- 任务创建与删除的后台作业机制已落地（P0 第 3、5 条）：创建、停止、结束、删除都有持久作业行、分段耗时与关联标识、失败重试与重启恢复；受理路径 p95 基线与慢模型/慢 Docker/故障注入**已补测**（创建 7.54ms / 结束 4.97ms / 删除 4.69ms / 读 16.3µs，真实栈读路径 p95 ≤ 50.5ms），并钉住两个已知限制：初始化期间的生命周期锁会阻塞其他流程的停止、作业执行器单飞会让慢初始化推迟其他流程的作业。
- 资产发现/漏洞扫描、漏洞收集、利用链推理、渗透测试、报告输出的业务改造与对应页面（计划书 P1–P6）：**均未开始**。原始运行日志和第三方错误内容不属于静态文案替换范围。
