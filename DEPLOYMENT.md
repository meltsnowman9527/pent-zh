...
- `pentagi` 应用容器最近一次重建于 2026-09-14 13:40:48（修复：流程详情页切页回来后不刷新，改为 cache-and-network），运行 `pentagi-local:latest`；`pgvector`、`scraper`、`pgexporter` 仍为原配套容器，原数据库卷未替换。
- 容器内 `/opt/pentagi/fe/index.html` 与本地 `frontend/dist/index.html` 校验值一致（md5 `463529d3bfa22203f5e4041e0d5b5c0c`），说明运行界面就是当前源码的构建产物。
