...
- `pentagi` 应用容器最近一次重建于 2026-09-14 14:40:18（提示词模板预设汉化、知识库内容语言跟随流程语言、详情导航标题汉化），运行 `pentagi-local:latest`；`pgvector`、`scraper`、`pgexporter` 仍为原配套容器，原数据库卷未替换。
- 容器内 `/opt/pentagi/fe/index.html` 与本地 `frontend/dist/index.html` 校验值一致（md5 `9314d60119c2a820645178f9571a473f`），说明运行界面就是当前源码的构建产物。
